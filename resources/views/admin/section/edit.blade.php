...
@section('section_header_breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ __('Memperbarui Data Bagian') }}</li>
@endsection

@section('page_content')
    <div class="row">
        <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12">
            <form method="POST" action="{{ route('admin.section.update', $section) }}">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex-wrap card-header d-flex align-items-center flex-xxl-nowrap">
                        <div class="flex-fill">
                            <div class="card-title">
                                {{ __('Memperbarui Data Bagian') }}
                                <p class="subtitle text-muted fs-12 fw-normal">
                                    {{ __('Silahkan input data untuk proses memperbarui data bagian') }}
                                </p>
                            </div>
                        </div>
                        <div class="d-flex">
                        ...
                        <div class="mb-4 row gy-4">
                            <div class="col-xl-12">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    ...
                                    placeholder="{{ __('Nama Bagian') }}" required autofocus>
                                @error('name')
                            ...
                            <div class="col-xl-12">
                                ...
                                <select
                                    ...
                                    @foreach ($departments as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ $key == $section->department_id ? 'selected' : '' }}>
                                            {{ $value }}</option>
                                    @endforeach
                                </select>
                                @error('department')
